...
import { Mail, Plus, Trash2, RefreshCw, Send, Clock, CheckCircle, AlertCircle, XCircle, BarChart2, Settings, History } from 'lucide-react';
...
export default function RFQDigestPage() {
  ...
  const [sendResult, setSendResult] = useState<{ status: string; sent_count: number; message: string } | null>(null);
  const [activeTab, setActiveTab] = useState<'config' | 'history' | 'preview'>('config');
  ...
  async function saveConfig() {
    ...
    try {
      ...
      if (res.ok) {
        const cfg: DigestConfig = await res.json();
        setConfig(cfg);
      }
    } catch {}
      ...
      if (res.ok) {
        ...
        setSendResult({
          ...
          message: d.status === 'success'
            ? `Digest sent to ${d.sent_count} recipient(s).`
            : d.status === 'failed' && d.reason === 'no_recipients'
            ? 'No recipients configured. Add at least one email first.'
            : `Partial send: ${d.sent_count} sent, ${d.failed_count} failed.`,
        });
        load();
      }
    } catch { setSendResult({ status: 'failed', sent_count: 0, message: 'Network error.' }); }
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#9aacbc] mt-0.5">
            Automated weekly email summary of RFQ performance sent to configured recipients
          </p>
        </div>
        <div className="flex items-center gap-2">
          {sendResult && (
            <span className={`text-xs ${sendResult.status === 'success' ? 'text-emerald-400' : sendResult.status === 'partial' ? 'text-amber-400' : 'text-rose-400'}`}>
              {sendResult.message}
            </span>
          )}
          <button
      ...
      {/* Status strip */}
      {config && (
        <div className="bg-[#1a232e] border border-[#2a3540] rounded-lg p-4 flex items-center gap-6">
          <div className="flex items-center gap-2">
            <div className={`w-2.5 h-2.5 rounded-full ${config.is_active ? 'bg-emerald-400' : 'bg-[#4a5c6a]'}`} />
            <span className="text-sm text-white">{config.is_active ? 'Active' : 'Paused'}</span>
          </div>
          ...
          {config.last_run_at && (
            <div className="text-xs text-[#4a5c6a] ml-auto">
              Last sent: {new Date(config.last_run_at).toLocaleString()}
            </div>
          )}
          {!config.last_run_at && (
            <div className="text-xs text-[#4a5c6a] ml-auto">Never sent yet</div>
          )}
        </div>
      )}
      ...
      <div className="flex gap-1 border-b border-[#2a3540]">
        {([
          ...
          { id: 'preview', label: 'Email Preview', icon: <BarChart2 size={13} /> },
          { id: 'history', label: `History${logs.length ? ` (${logs.length})` : ''}`, icon: <History size={13} /> },
        ] as const).map((tab) => (
          <button
            key={tab.id}
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="bg-[#1a232e] border border-[#2a3540] rounded-lg p-5 space-y-4">
            ...
            <div className="space-y-2">
              ...
              {emails.length === 0 && (
                <p className="text-xs text-[#4a5c6a] text-center py-3">No recipients yet. Add one below.</p>
              )}
            </div>
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 text-xs rounded transition-colors ${
                        schedDay === i
                          ...
                          : 'bg-[#0f1419] text-[#9aacbc] hover:text-white'
                      }`}
                    >
                      {d.slice(0, 3)}
                    </button>
      ...
      {activeTab === 'preview' && (
        <div className="space-y-4">
          <p className="text-xs text-[#9aacbc]">
            This is the content that will be included in the next digest email, based on the last {windowDays} days of data.
          </p>
          {loading || !stats ? (
                ...
                {logs.map((log) => {
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-[#1e2a34] transition-colors">
                      ...
                      <td className="p-4">
                        <span className="text-sm text-[#9aacbc]">{(log.recipients || []).join(', ') || '—'}</span>
                      </td>
                      ...
                      <td className="p-4">
                        {snap && typeof snap.total_broadcasts === 'number' ? (
                          <span className="text-xs text-[#9aacbc]">
                            {snap.total_broadcasts} broadcasts · {snap.response_rate_pct as number}% response
                            {snap.avg_unit_price_usd ? ` · $${(snap.avg_unit_price_usd as number).toFixed(2)} avg` : ''}
                          </span>
                        ) : <span className="text-xs text-[#4a5c6a]">—</span>}
                      </td>
                    </tr>
                  );
                })}
